 ...
 * IJettyService
 * 
 * Android Service which runs the Jetty server, maintaining it in the active Notifications so that the user can return
 * to the IJetty Activity to control it at any time.
 */
public class IJettyService extends Service
{
    ...
    { "org.mortbay.ijetty.webapp.AndroidWebInfConfiguration", "org.eclipse.jetty.webapp.WebXmlConfiguration",
            ...
            public void handleMessage(Message msg)
            {
                switch (msg.getData().getInt("state"))
                {
                    case __STARTED:
                    {
                        ...
                        PendingIntent contentIntent = PendingIntent.getActivity(IJettyService.this,0,new Intent(IJettyService.this,IJetty.class),0);
    ...
    public void configureRealm() throws IOException
    {
        ...
        if (realmProps.exists())
        {
            ...
            if (_consolePassword != null)
                realm.putUser("admin",Credential.getCredential(_consolePassword),new String[]
                { "admin" }); //set the admin password for console webapp
